                      ...
                      <div class="form-grid" style="grid-template-columns: 1fr 1fr;">
                          ...
                          <div>
                              ...
                              <select name="timezone" class="w-full h-10 rounded-lg border border-border-color bg-bg-surface px-3 text-[13px] text-text-primary focus:outline-none focus:border-brand-primary">
                                  <option value="Africa/Casablanca" <?php echo e(auth()->user()->timezone == 'Africa/Casablanca' ? 'selected' : ''); ?>>Africa/Casablanca (GMT+1)</option>
                                  <option value="Europe/Paris" <?php echo e(auth()->user()->timezone == 'Europe/Paris' ? 'selected' : ''); ?>>Europe/Paris (GMT+1/GMT+2)</option>
                              </select>
                          </div>
                          <div>
                              ...
                              <select name="date_format" class="w-full h-10 rounded-lg border border-border-color bg-bg-surface px-3 text-[13px] text-text-primary focus:outline-none focus:border-brand-primary">
                                  <option value="d/m/Y" <?php echo e(auth()->user()->date_format == 'd/m/Y' ? 'selected' : ''); ?>>JJ/MM/AAAA (ex: 31/12/2026)</option>
                                  <option value="Y-m-d" <?php echo e(auth()->user()->date_format == 'Y-m-d' ? 'selected' : ''); ?>>AAAA-MM-JJ (ex: 2026-12-31)</option>
                              </select>
                          </div>
                          <div>
                      ...
                      <div class="space-y-4">
                          <div class="form-grid" style="grid-template-columns: 1fr 1fr;">
                              <div>
                                  ...
                                  <input type="text" name="name" value="<?php echo e(old('name', auth()->user()->name)); ?>" class="w-full h-10 rounded-lg border border-border-color bg-bg-surface px-3 text-[13px] text-text-primary focus:outline-none focus:border-brand-primary">
                              </div>
                              <div>
                                  <label class="block text-[13px] font-medium text-text-primary mb-1">Email de contact</label>
                                  <input type="email" name="email" value="<?php echo e(old('email', auth()->user()->email)); ?>" class="w-full h-10 rounded-lg border border-border-color bg-bg-surface px-3 text-[13px] text-text-primary focus:outline-none focus:border-brand-primary">
                              </div>
                              <div>
                                  <label class="block text-[13px] font-medium text-text-primary mb-1">Téléphone</label>
                                  <input type="text" name="phone" value="<?php echo e(old('phone', auth()->user()->phone)); ?>" placeholder="+212 6..." class="w-full h-10 rounded-lg border border-border-color bg-bg-surface px-3 text-[13px] text-text-primary focus:outline-none focus:border-brand-primary">
                              </div>
                          </div>

                          <div>
                              <label class="block text-[13px] font-medium text-text-primary mb-1">Adresse</label>
                              <textarea name="address" rows="2" class="w-full rounded-lg border border-border-color bg-bg-surface px-3 py-2 text-[13px] text-text-primary focus:outline-none focus:border-brand-primary"><?php echo e(old('address', auth()->user()->address)); ?></textarea>
                          </div>
                      </div>
