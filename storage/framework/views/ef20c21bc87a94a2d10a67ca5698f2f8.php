...
async function suggestCategoryWithAI() {
    ...
    try {
        ...
        badgeIcon.className = `fas ${isHigh ? 'fa-circle-check' : isMedium ? 'fa-triangle-exclamation' : 'fa-circle-xmark'} mt-0.5 flex-shrink-0`;
        badgeText.textContent = found
            ? `✨ IA suggère : ${data.category_name} (${pct}% confiance)`
            : `⚠️ Catégorie "${data.category_name}" non trouvée dans la liste`;
        badgeReason.textContent = data.reason;
