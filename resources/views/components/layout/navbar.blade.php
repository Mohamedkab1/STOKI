{{-- 
    Navbar Supérieure (Top Navigation Bar)
    Contient: toggle sidebar mobile, date, toggle thème, cloche notifications, menu utilisateur
--}}
...
<header class="app-navbar">
    ...
    <div class="hidden lg:flex items-center text-[10px] font-black uppercase tracking-[0.2em] text-text-muted opacity-60">
        <span class="capitalize">{{ now()->locale('fr')->isoFormat('dddd D MMMM YYYY') }}</span>
    </div>
        ...
        <div class="relative" x-data="{ open: false }" @click.away="open = false">
            <button @click="open = !open" class="flex items-center gap-3 p-1 rounded-xl hover:bg-sidebar-active transition-all group border-0 bg-transparent cursor-pointer">
                ...
                <div class="hidden md:block text-left pr-2">
                    <p class="text-[11px] font-black text-text-main leading-tight uppercase tracking-tight m-0" style="color: var(--text-primary)">{{ auth()->user()->name }}</p>
            ...
            >
                <div class="px-5 py-4 border-b border-border-subtle bg-sidebar-active/50">
                    ...
                    <p class="text-xs font-bold text-text-main truncate" style="color: var(--text-primary); margin:0;">{{ auth()->user()->email }}</p>
                </div>
                
                <div class="p-2.5">
                    <a href="{{ route('profile') }}" class="flex items-center gap-3 px-4 py-3 text-[11px] font-black uppercase tracking-tight text-text-muted hover:text-brand-primary hover:bg-brand-primary/5 rounded-xl transition-all group" style="text-decoration:none;">
